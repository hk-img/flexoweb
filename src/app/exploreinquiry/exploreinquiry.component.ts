...
@Component({
  selector: 'app-exploreinquiry',
  templateUrl: './exploreinquiry.component.html',
  styleUrls: ['./exploreinquiry.component.css'],
})
export class ExploreinquiryComponent {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ExploreinquiryComponent>,
  ...
  onCountryCodeChange(country: any) {
    ...
    this.selectedIndex = this.countryCodes.findIndex(
      (code: any) => code?.dialcode === country
    );
    this.updatePlaceholder();
  }
  ...
  updatePlaceholder() {
    if (this.selectedIndex) {
      this.placeholder = Array.from(
        {
          length:
            this.countryCodes[this.selectedIndex]['number-of-digits-in-number'],
        },
        (_, i) => i
      ).join('');
    }
  ...
  onSubmit(formData: any) {
    ...
    if (isPlatformBrowser(this.platformId)) {
      ...
      if (this.profileDetailForm.form.status == 'VALID') {
        ...
        this.service.inquiryBooking(formData).subscribe(
          (data: any) => {
            if (data?.result?.success) {
              this.popupOpen(
                'inquirenow',
                `${data?.result?.message}. Our team will get back to you shortly.`
              );
